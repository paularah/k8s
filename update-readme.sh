...
has_images() {
  local folder="$1"
  if [ -n "$(find "${folder}" -maxdepth 1 -type f -iname '*.png' -o -iname '*.jpg' -o -iname '*.jpeg' -o -iname '*.gif' -o -iname '*.bmp' -o -iname '*.svg' -o -iname '*.webp' 2>/dev/null)" ]; then
    return 0
  else
    return 1
  fi
...
append_images() {
  local folder="$1"
  for image in "${folder}"/*.{png,jpg,jpeg,gif,bmp,svg,webp} 2>/dev/null; do
